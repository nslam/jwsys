...
def dotest(request, paper_id):
    ...
    if request.method == 'GET':
        return render(request, 'test.html',
                      {'paper': paper, 'question_pd_list': question_pd_list, 'question_xz_list': question_xz_list})
    else:
...
def student2(request):
    ...
    return render(request, 'student2.html',
                  {'user': user, 'course_list': takes, 'allcourse_score_list': allcourse_score_list})
...
def newpdquestion(request):
    ...
    if 'course_name' not in request.POST:
        return HttpResponse('<script>alert("没有输入课程！");location.replace("/onlinetest/teacher3/");</script>')
    else:
        qcours = request.POST['course_name']
        try:
            cour = Course.objects.get(title=qcours)
        except ObjectDoesNotExist:
            return HttpResponse('<script>alert("没有这个课程！");location.replace("/onlinetest/teacher3/");</script>')
        qcourse = Course.objects.get(title=qcours)

    if 'question_title' not in request.POST:
        return HttpResponse('<script>alert("没有输入题目！");location.replace("/onlinetest/teacher3/");</script>')
    else:
        qtitle = request.POST['question_title']

    if 'question_ans' not in request.POST:
        return HttpResponse('<script>alert("没有输入答案！");location.replace("/onlinetest/teacher3/");</script>')
    else:
    ...
    if 'test_point' not in request.POST:
        return HttpResponse('<script>alert("没有输入考点！");location.replace("/onlinetest/teacher3/");</script>')
    else:
        qtestpoi = request.POST['test_point']

    if 'difficulty' not in request.POST:
        return HttpResponse('<script>alert("没有输入题目难度！");location.replace("/onlinetest/teacher3/");</script>')
    else:
...
def newxzquestion(request):
    ...
    if 'choice1' not in request.POST:
        return HttpResponse('<script>alert("没有输入选项A！");location.replace("/onlinetest/teacher3/");</script>')
    else:
        ca = request.POST['choice1']

    if 'choice2' not in request.POST:
        return HttpResponse('<script>alert("没有输入选项B!");location.replace("/onlinetest/teacher3/");</script>')
    else:
        cb = request.POST['choice2']

    if 'choice3' not in request.POST:
        return HttpResponse('<script>alert("没有输入选项C！");location.replace("/onlinetest/teacher3/");</script>')
    else:
        cc = request.POST['choice3']

    if 'choice4' not in request.POST:
        return HttpResponse('<script>alert("没有输入选项D！");location.replace("/onlinetest/teacher3/");</script>')
    else:
    ...
    return render(request, 'teacher3.html',
                  {'allquestion_list': ques, 'user': tuser, 'myquestion_list': myques})  # ,{'success':insertSuccess}
...
def deletequestion(request, did):
    if did:
        ...
        return render(request, 'teahcer3.html', {'error': '未获得题目id'})  # 未得到题目信息
...
def changequestion(request, updID):
    if updID:
        ...
        if theQues:
            ...
            error = '未获得题目id'
            return render(request, 'teacher3.html', {'error': error})  # error
...
def searchquestion(request):
    if 'question_title' not in request.POST:  # attr等名字由网页页面决定
        error = '没有输入题目！'
        return render(request, 'teahcer3.html', {'error': error})  # 返回本页面，附加错误信息
    else:
...
def paper2(request, paperId):
    ...
    if type != 'Instructor':
        ...
        return render(request, 'paper2.html',
                      {'user': user, 'paper': paper, 'question_pd_list': pd, 'question_xz_list': xz,
...
def paperchinfo(request, paperId):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return HttpResponse('<script>alert("没有这个课程！");location.replace("/onlinetest/paper1/");</script>')
...
def newpaper(request):
    ...
    if ifauto == 'yes':
        ...
        if pdqueslist.count() < int(pdnum):
            ...
            if pdqueslist.count() < int(pdnum):
                return HttpResponse('<script>alert("没有这么多判断题！");location.replace("/onlinetest/paper1/");</script>')
        if xzqueslist.count() < int(xznum):
            xzqueslist = Question.objects.filter(course_id=p.course_id, q_type='xz')
            if xzqueslist.count() < int(xznum):
                return HttpResponse('<script>alert("没有这么多选择题！");location.replace("/onlinetest/paper1/");</script>')
    ...
    if type != 'Instructor':
        ...
        return render(request, 'paper2.html',
                      {'user': user, 'paper': paper, 'question_pd_list': question_pd_list,
                       'question_xz_list': question_xz_list, 'allquestion_list': questions})


def statistics(request):
    path = os.path.abspath('.')
    error = 0
    if 'paper' in request.POST:  # 输入一级条件是试卷的name
        paper = request.POST['paper']
        pap = Paper.objects.get(name=paper)
        if 'test_point' in request.POST and request.POST['test_point'] != '':
            error += 1
            questions = pap.question_set.all()
            sheets = Sheet.objects.filter(paper_id=pap.id)
            tps_tot = {}  # 题目的考点及对应总的题数的字典
            tps_get = {}  # 某场考试某考点对的题数
            for q in questions:  # 试题的所有考点们
                t = q.test_point
                if t in tps_tot.keys():  # 已经出现过的考点
                    tps_tot[t] += 1
                else:  # 未重复的新的考点
                    tps_tot[t] = 0
                    tps_get[t] = 0
            for s in sheets:
                for q in questions:
                    re = Reply.objects.filter(sheet_id=s.id).filter(question_id=q.id)
            ...
            autolabel(rects)
            plt.savefig('statistics.jpeg')
            return render(request, 'teacher2.html',
                          {'error': paper, 'picture': path + '/statistics.jpeg', 'total_pd': "", 'total_xz': "",
                           'avgage': "", 'smax': "", 'smin': ""})
        elif 'type' in request.POST and request.POST['type'] != '':  # 返回2种类型的总的分数
            error += 2
            sheets = Sheet.objects.filter(paper_id=pap.id)
            pd_tot = 0
            xz_tot = 0
            for s in sheets:
                ...
                xz_tot += s.xz_mark
            return render(request, 'teacher2.html',
                          {'error': error, 'picture': '', 'total_pd': pd_tot, 'total_xz': xz_tot, 'avgage': "",
                           'smax': "", 'smin': ""})
            ...
            plt.savefig('statistics.jpeg')
            return render(request, 'teacher2.html',
                          {'error': error, 'picture': path + '/statistics.jpeg', 'total_pd': "", 'total_xz': "",
                           'avgage': avg, 'smax': smax, 'smin': smin})
    elif 'student' in request.POST:
        error += 8
        student = request.POST['student']
        sid = User.objects.get(username=student).student
        theSheets = Sheet.objects.filter(student_id=sid.id)
        scorelist = {}
        for s in theSheets:
            pid = s.paper
            pname = pid.name
        ...
        error = '请输入试卷或学生'
        return render(request, 'teacher2.html',
                      {'error': error, 'picture': "", 'total_pd': "", 'total_xz': "", 'avgage': "", 'smax': "",
                       'smin': ""})
